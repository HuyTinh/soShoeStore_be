...
public interface ShoeRepository extends JpaRepository<Shoe, Long> {
    ...
    @Query(value = """
            ...
            select FORMAT(DayDate, 'yyyy-MM-dd') as 'date' ,(SELECT ISNULL(SUM(total_money), 0) FROM orders WHERE MONTH(order_date) = MONTH(DayDate) AND DAY(order_date) = DAY(DayDate) AND YEAR(order_date) = YEAR(DayDate) AND status LIKE 'APPROVE' AND order_id in (SELECT order_id FROM order_details WHERE shoe_id = ?3)
    ...
    @Query(value = """
            DECLARE @shoe_categories  TABLE  (
                         ...
                         SELECT title , (SELECT  TOP 1  image_url FROM shoes WHERE name LIKE CONCAT(title, '%')
                       ) FROM @shoe_categories""", nativeQuery = true)
    ...
    @Query(value = """
            DECLARE @shoe_sales TABLE(
                          shoe_id INT NOT NULL,
                          sales FLOAT NOT NULL,
                          order_count INT NOT NULL
                        )
                        INSERT INTO  @shoe_sales
                        SELECT TOP 10 shoe_id, SUM(total_money), COUNT(shoe_id) FROM order_details WHERE order_id IN (SELECT order_id FROM orders WHERE YEAR(order_date) = YEAR(GETDATE()) AND MONTH(order_date) = ?1) GROUP BY shoe_id ORDER BY COUNT(shoe_id) DESC
